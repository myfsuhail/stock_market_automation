...
def fetch_stock_ids_from_db():
    try:
        ...
        where pair_id not in (
        	select distinct pair_id from buy_sell_buddy.public.historical_stock_data_weekly
        )       
        """
...
def load_csv_to_postgres(n, file_path, table_name='historical_stock_data_weekly'):
    try:
        conn = psycopg2.connect(
            dbname="buy_sell_buddy",
            user="admin",
...
def truncate_table():
    try:
        ...
        cursor = conn.cursor()
        truncate_query = "truncate table historical_stock_data_weekly"
        cursor.execute(truncate_query)
        conn.commit()
        print("Table historical_stock_data_weekly truncated successfully.")
        conn.close()
    except Exception as e:
