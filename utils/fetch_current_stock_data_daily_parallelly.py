...
def load_csv_to_duckdb(n, file_path, duckdb_path='/app/stock_automate/buy_sell_buddy.duckdb', table_name='current_stock_data_daily'):
    try:
        conn = duckdb.connect(duckdb_path)
        df = pd.read_csv(file_path)
...
if __name__ == '__main__':
    ...
    truncate_table_duckdb('/app/stock_automate/buy_sell_buddy.duckdb', 'current_stock_data_daily')
    ...
    while True:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:  # Adjust max_workers based on your system capacity
